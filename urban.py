    ...
    def step(self, model):
        if not self.settled:
            ...
            if best_tile == l_tile.info["value"]:
                self.direction += RNG(self.search_angle)
            if best_tile == r_tile.info["value"]:
                self.direction -= RNG(self.search_angle)
            self.forward()

            max_attraction_value = 255
            settle_value = RNG(int(t.info["value"]))
...
def setup(model):
    ...
    tx = model.x_tiles
    ty = model.y_tiles
    init_tile = model.tiles[0]

    # Generate the "terrain"
    model.tiles[0].info["value"] = 100+RNG(100)
    for x in range(tx):
        for y in range(ty):
            avg_value = 0
            neighbors = 0
            ...
            if neighbors > 0:
                i = y*tx+x
                model.tiles[i].info["value"] = max(0,min(255,avg_value//neighbors-20+RNG(40)))
